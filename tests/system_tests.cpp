...
TEST(RvcSystemTest, SimulatorRestoresHeadingBeforeBackward) {
    ...
    EXPECT_TRUE(containsLog(result.logs, "motion=TurnRight cleaner=Normal"));
    EXPECT_TRUE(containsLog(result.logs, "motion=TurnLeft cleaner=Normal"));
    EXPECT_TRUE(containsLog(result.logs, "motion=Backward"));
    EXPECT_TRUE(containsLog(result.logs, "motion=Backward cleaner=Normal"));
...
TEST(RvcSystemTest, SimulatorKeepsCleanerOnDuringBoostedEscape) {
    GridSimulator simulator = GridSimulator::fromLines({
        "#######",
        "#>*..##",
        "###.###",
    ...
    const rvc::SimulationResult result = simulator.run(7, false);

    EXPECT_TRUE(containsLog(result.logs, "motion=TurnRight cleaner=Boost"));
    EXPECT_TRUE(containsLog(result.logs, "motion=TurnLeft cleaner=Normal"));
    EXPECT_TRUE(containsLog(result.logs, "motion=Backward cleaner=Normal"));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ScenarioExpectation{"backward_escape.rvc", 4, Position{3, 2}, Direction::West, 0, 0, 0, 2, 1, 1, 0, 0,
                            {"right probe blocked: original heading restored, backing up"}},
        ScenarioExpectation{"backward_escape2.rvc", 20, Position{8, 3}, Direction::East, 0, 0, 1, 6, 7, 6, 0, 0,
        ...
        ScenarioExpectation{"dense_dust_maze_extreme.rvc", 50, Position{3, 5}, Direction::East, 7, 0, 38, 10, 2, 0,
                            11, 0, {"motion=Forward cleaner=Boost", "motion=TurnLeft cleaner=Normal"}},
        ScenarioExpectation{"dust_and_interrupt.rvc", 10, Position{1, 2}, Direction::West, 2, 0, 8, 2, 0, 0, 3, 0,
                            {"dustPeriodic=detected"}},
        ScenarioExpectation{"dust_before_dead_end_escape.rvc", 8, Position{2, 3}, Direction::South, 1, 0, 4, 1, 2, 1,
                            2, 0, {"motion=Backward cleaner=Normal"}},
        ScenarioExpectation{"dust_trail_boost_refresh.rvc", 8, Position{1, 8}, Direction::South, 3, 0, 7, 0, 1, 0,
                            6, 0, {"motion=Forward cleaner=Boost"}},
        ScenarioExpectation{"front_both_sides_open.rvc", 3, Position{2, 1}, Direction::South, 0, 0, 1, 2, 0, 0, 0,
                            0, {"motion=TurnLeft cleaner=Normal"}},
        ScenarioExpectation{"front_clears_but_sides_still_blocked.rvc", 6, Position{1, 2}, Direction::West, 0, 0, 2,
                            2, 1, 1, 0, 0, {"right probe blocked: original heading restored, backing up"}},
        ScenarioExpectation{"front_left_only_open.rvc", 2, Position{2, 1}, Direction::West, 0, 0, 1, 1, 0, 0, 0, 0,
                            {"motion=TurnLeft cleaner=Normal"}},
        ScenarioExpectation{"front_right_only_open.rvc", 2, Position{2, 3}, Direction::East, 0, 0, 1, 0, 1, 0, 0, 0,
                            {"right probe open: resume forward cleaning"}},
        ScenarioExpectation{"large_open_room_dust_sweep.rvc", 30, Position{5, 1}, Direction::West, 3, 2, 28, 0, 2, 0,
                            9, 0, {"dustPeriodic=detected"}},
        ScenarioExpectation{"long_escape_left_exit_extreme.rvc", 22, Position{9, 2}, Direction::West, 0, 0, 0, 8, 7,
                            7, 0, 0, {"escaping: left opened, turn toward exit"}},
        ScenarioExpectation{"long_escape_right_exit_extreme.rvc", 23, Position{9, 3}, Direction::East, 0, 0, 1, 7, 8,
                            7, 0, 0, {"right probe open: resume forward cleaning"}},
        ScenarioExpectation{"narrow_tunnel_sides_blocked_front_clear.rvc", 6, Position{1, 6}, Direction::South, 0, 0,
                            5, 0, 1, 0, 0, 0, {"leftPeriodic=blocked dustPeriodic=clear motion=Forward"}},
        ScenarioExpectation{"ragged_map_edge_extreme.rvc", 8, Position{0, 0}, Direction::West, 1, 0, 6, 2, 0, 0, 2, 0,
                            {"frontInterrupt=true"}},
        ScenarioExpectation{"repeated_front_interrupts_alternation.rvc", 12, Position{2, 5}, Direction::East, 0, 0, 6,
                            4, 1, 1, 0, 0, {"motion=TurnLeft cleaner=Normal", "motion=TurnRight cleaner=Normal"}},
        ScenarioExpectation{"sealed_box_extreme.rvc", 10, Position{2, 2}, Direction::East, 0, 0, 0, 3, 4, 3, 0, 0,
                            {"escaping: left blocked, probe right"}}),
    scenarioName);
