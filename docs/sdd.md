...
| Controller | `include/rvc/RvcController.hpp`, `src/RvcController.cpp` | [변경] 실행 상태와 interrupt 소비를 조율하고 navigation/power 정책 결과를 command로 조합하며 hardware adapter를 직접 알지 않는다. |
| Navigation Policy | `include/rvc/NavigationPolicy.hpp`, `src/NavigationPolicy.cpp` | [추가] 전방/좌측 장애물과 우측 탐색 상태를 기반으로 다음 motion과 회피/탈출 상태 전이를 결정한다. |
| Cleaning Power Policy | `include/rvc/CleaningPowerPolicy.hpp`, `src/CleaningPowerPolicy.cpp` | [추가] 먼지 감지 후 boost 유지 tick을 관리하고 motion 판단과 독립적으로 cleaner power 후보를 결정한다. |
...
| `RvcController::tick(const PeriodicSensorData&)` | periodic sensor 값 | `Command` | [R2-변경] 제어 tick마다 좌측/먼지 sensor 값과 우측 탐색 상태를 반영하고 navigation/power 정책 결과를 조합해 다음 command를 반환한다. | FR-06 to FR-15 |
...
| `RvcController::decideNextCommand(const SensorSnapshot&)` | sensor snapshot | `Command` | [R2-변경] `NavigationPolicy`가 결정한 motion과 `CleaningPowerPolicy`가 결정한 cleaner power 후보를 조합한다. | FR-07 to FR-15 |
...
8. `decideNextCommand()`가 `NavigationPolicy`의 motion 판단과 `CleaningPowerPolicy`의 cleaner power 판단을 조합해 command를 결정한다.
...
`CleaningPowerPolicy::update(bool dustDetected)`는 motion 판단과 독립적으로 cleaner power 후보를 결정한다. 먼지가 감지되면 내부 boost 잔여 tick을 configured duration으로 재설정한다. 먼지가 감지되지 않고 남은 boost tick이 있으면 1 감소시킨다. 남은 boost tick이 0보다 크면 `Boost`, 아니면 `Normal`을 반환한다. 최종 `Command` 생성 시에는 `Forward` 동작에서만 해당 power를 출력하고, 회피 회전, 후진, 정지 동작에서는 cleaner power를 `Off`로 내보낸다.
...
| 테스트 수준 | 대상 | 검증 내용 |
| --- | --- | --- |
| Controller unit test | `RvcController` | [R2-변경] 전진, 중지, interrupt 소비, navigation/power 정책 조합, 회피 중 cleaner off |
| Policy unit test | `NavigationPolicy`, `CleaningPowerPolicy` | [추가] 좌측 회피, 우측 탐색 성공/실패, 원래 방향 복구 후 후진, dust boost duration과 reset을 독립적으로 검증한다. |
...
| FR-14 | `CleaningPowerPolicy::update`, `ControllerConfig::dustBoostTicks` | `DustBoostLastsConfiguredTicks`, `DustRefreshesAndAgesBoostBudget`, `AvoidanceOutputStaysOffWhileBoostStateIsMaintained`, `SimulatorCleansDustAndLogsCommands`, `SimulatorKeepsCleanerOffDuringBoostedEscape` |
| FR-15 | `CleaningPowerPolicy`의 boost 잔여 tick 감소와 `CleaningPower::Normal` 복귀 | `DustBoostLastsConfiguredTicks`, `DustRefreshesAndAgesBoostBudget`, `AvoidanceOutputStaysOffWhileBoostStateIsMaintained` |
...
| `NavigationPolicy`와 `CleaningPowerPolicy`를 `RvcController`에서 분리한다. | 방향 결정과 dust boost 예산 관리를 독립 책임으로 분리해 `RvcController`의 SRP 위반을 줄이고 정책 단위 테스트를 가능하게 하기 위해서이다. |
...
| `decideNextCommand()`는 navigation 판단과 power 판단을 조합하는 지점으로 둔다. | 회피/탈출 상태 전이는 `NavigationPolicy`, boost 판단은 `CleaningPowerPolicy`에서 독립적으로 테스트하고 controller는 조합 책임만 검증하기 위해서이다. |
