...
이 다이어그램의 `Motor`, `Cleaner`, sensor 항목은 문제 영역의 도메인 개념이며 C++ 구현 클래스를 의미하지 않는다. [변경] 설계 문서의 구현 관점에서는 상위 객체 `Rvc`가 `RvcController`와 `RvcHardwareAdapter`를 소유하고, `GridSimulator`는 `SimulatedHardwareAdapter`를 통해 검증용 하드웨어 환경만 제공한다. [R2-변경] 우측 장애물 판단은 별도 RightSensor가 아니라 RVC의 `TurnRight` 동작 후 FrontSensor interrupt로 확인한다. [R3-추가] 후진 주행 중 후방 장애물은 RearSensor interrupt로 알린다.
...
- [R3-추가] RearSensor도 polling 대상이 아니라 후진 주행 중 사용되는 interrupt source이다.
...
- [R2-기존] 회피/탈출 이동인 `Backward`, `TurnLeft`, `TurnRight` 동안 cleaner output은 `Off`이며, `Forward` 전진 청소에서만 `Normal` 또는 `Boost`가 적용된다.
- [R3-변경] R3 정책에서는 청소 중 모든 주행/회피/탈출 상태의 기본 cleaner output을 `Normal`로 유지하고, `Boost`는 먼지 감지로 필요한 제자리 회전 구간에만 적용한다.
- [R2-변경] `Escaping` 상태에서는 `Backward`를 수행하고, 매 후진 후 좌측이 막혀 있으면 우측 탐색을 반복한다. 탈출 조건은 좌측 open 또는 우측 탐색 open이다.
- [R3-추가] 후진 주행 중 후방 장애물이 감지되면 rear interrupt를 기준으로 현재 주행 방향의 장애물 회피 정책을 적용한다.
- [R3-변경] 먼지는 `Boost` 청소 후에도 persistent fixture로 남으며, 재방문 시 다시 감지될 수 있다.
