...
이 다이어그램의 `Motor`, `Cleaner`, sensor 항목은 문제 영역의 도메인 개념이며 C++ 구현 클래스를 의미하지 않는다. [변경] 설계 문서의 구현 관점에서는 상위 객체 `Rvc`가 `RvcController`와 `RvcHardwareAdapter`를 소유하고, `GridSimulator`는 `SimulatedHardwareAdapter`를 통해 검증용 하드웨어 환경만 제공한다.
